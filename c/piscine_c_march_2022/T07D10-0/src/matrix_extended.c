...
void output_max_in_row_dynamic(int **arr, int x, int y);
void output_max_in_row_static(int arr[ARR_SIZE][ARR_SIZE], int x, int y);
void output_min_in_col_dynamic(int **arr, int x, int y);
void output_min_in_col_static(int arr[ARR_SIZE][ARR_SIZE], int x, int y);
...
int static_arr(int size_x, int size_y) {
    ...
    output_max_in_row_static(static_arr, size_x, size_y);
    output_min_in_col_static(static_arr, size_x, size_y);
    return (1);
}
...
int dynamic_single_matrix_arr(int size_x, int size_y) {
    ...
    output_max_in_row_dynamic(arr, size_x, size_y);
    output_min_in_col_dynamic(arr, size_x, size_y);
    free(arr);
    return (1);
}
...
int array_of_pointers(int size_x, int size_y) {
    ...
    output_max_in_row_dynamic(ptr_arr, size_x, size_y);
    output_min_in_col_dynamic(ptr_arr, size_x, size_y);
    free(ptr_arr);
    free(val_arr);
    return (1);
}
...
void output_max_in_row_dynamic(int **arr, int x, int y) {
    ...
    while (count_x < x) {
        ...
        count_y = 0;
        max = arr[count_x][count_y];
        while (count_y < y) {
            if (arr[count_x][count_y] > max)
                max = arr[count_x][count_y];
            count_y++;
        }
        printf("%d", max);
...
void output_min_in_col_dynamic(int **arr, int x, int y) {
    ...
    while (count_y < y) {
        ...
        count_x = 0;
        min = arr[count_x][count_y];
        while (count_x < x) {
            if (arr[count_x][count_y] < min)
                min = arr[count_x][count_y];
            count_x++;
        }
        printf("%d", min);
